...
impl Solution {
    ...
    fn dfs(x: i32, y: i32, board: &mut Vec<Vec<char>>, step: usize, word: &[char]) -> bool {
        if x < 0
            ...
            || y < 0
            || y >= board.len() as i32
            || board[y as usize][x as usize] != word[step]
        {
            return false;
        }
        let mut tmp = '/';
        ...
        let res = Self::dfs(x - 1, y, board, step + 1, word)
            || Self::dfs(x + 1, y, board, step + 1, word)
            || Self::dfs(x, y - 1, board, step + 1, word)
            || Self::dfs(x, y + 1, board, step + 1, word);
...
fn check() {
    assert_eq!(
        Solution::exist(vec![vec!['a', 'b'], vec!['c', 'd']], "abdc".to_string()),
        true
    );
    assert_eq!(
        Solution::exist(vec![vec!['a', 'b'], vec!['c', 'd']], "abcd".to_string()),
        false
    );
