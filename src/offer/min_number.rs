...
impl Solution {
    ...
    fn varient_quick_sort(start :usize,end :usize,nums :&mut Vec<i32>){
        if start < end{
            ...
            let base = nums[start];
            while i!=j{
                while i < j && Self::compare2(base,nums[j]){//从右往左 当找到比base小的交换
                    j-=1;
                }
                nums[i] = nums[j];
                while i < j && Self::compare2(nums[i],base){//从左往右 当找到比base大的交换
                    i+=1;
                }
                nums[j] = nums[i];
            }
    ...
    fn compare(origin :i32 , other :i32)->bool{//该合并的 , 写半天写的什么东西。简直多此一举
...
fn check() {
    ...
    assert_eq!(Solution::min_number(vec![3,30,34,5,9]),"3033459".to_string());
