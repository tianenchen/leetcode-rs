...
impl Solution {
    ...
    pub fn is_rectangle_overlap_trash(rec1: Vec<i32>, rec2: Vec<i32>) -> bool {
        if rec1 == rec2
            || (rec1[0] < rec2[0] && rec1[1] < rec2[1] && rec1[2] > rec2[2] && rec1[3] > rec2[3])
            || (rec1[0] > rec2[0] && rec1[1] > rec2[1] && rec1[2] < rec2[2] && rec1[3] < rec2[3])
        {
            return true;
        }
...
fn check() {
    ...
    assert_eq!(
        Solution::is_rectangle_overlap(vec![5, 15, 8, 18], vec![0, 3, 7, 9]),
        false
    );
    assert_eq!(
        Solution::is_rectangle_overlap(vec![229, -132, 833, 333], vec![-244, -577, 837, 804]),
        true
    );
}
