...
impl Solution {
    ...
    pub fn can_three_parts_equal_sum2(a: Vec<i32>) -> bool {
        ...
        while j < len {
            let (sum0, mut sum1, mut sum2) = (
                a[0..=i].iter().sum::<i32>(),
                a[i + 1..j].iter().sum::<i32>(),
                a[j..len].iter().sum::<i32>(),
            );
            // dbg!(sum0,sum1,sum2);
...
mod tests {
    ...
    fn check() {
        ...
        assert_eq!(
            Solution::can_three_parts_equal_sum(vec![1, 4, 5, 4, 1]),
            true
        );
        assert_eq!(
        ...
        assert_eq!(
            Solution::can_three_parts_equal_sum(vec![6, 1, 1, 13, -1, 0, -10, 20]),
            false
        );
    }
}
