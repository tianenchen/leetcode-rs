...
impl Solution {
    pub fn can_measure_water(x: i32, y: i32, z: i32) -> bool {
        ...
        while let Some((remain_x, remain_y)) = stack.pop_back() {
            ...
            stack.push_back((
                remain_x - std::cmp::min(remain_x, y - remain_y),
                remain_y + std::cmp::min(remain_x, y - remain_y),
            ));
            stack.push_back((
                remain_x + std::cmp::min(remain_y, x - remain_x),
                remain_y - std::cmp::min(remain_y, x - remain_x),
            ));
        }
        false
    }
